...
CREATE TABLE PRODUCT (
	...
	ON UPDATE CASCADE ON DELETE RESTRICT	-- product should be deleted before removing crystal in case of discontinued product
);
...
CREATE TABLE WAREHOUSE (
    ...
    ON UPDATE CASCADE ON DELETE RESTRICT	-- address of warehouse should not be deleted unless warehouse is deleted/sold
);
...
CREATE TABLE	REWARDS_MEMBER (
    ...
    ON UPDATE CASCADE ON DELETE CASCADE, -- deleted customer should be removed
...
CREATE TABLE	INVENTORY (
    ...
    CONSTRAINT		InventoryFKWarehouse FOREIGN KEY	(WarehouseName)				REFERENCES		WAREHOUSE(WarehouseName)
